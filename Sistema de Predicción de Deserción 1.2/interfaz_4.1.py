...
class AppMejorada:
    ...
    def crear_interfaz(self):
        ...
        btn_entrenar = ctk.CTkButton(frame_botones1, text="1. Generar Datos Sintéticos", command=self.entrenar,
                                     fg_color="#2E7D32", hover_color="#1B5E20", height=45, font=ctk.CTkFont(size=14, weight="bold"))
        btn_entrenar.pack(side="left", padx=10, expand=True, fill="x")
        btn_cargar_excel = ctk.CTkButton(frame_botones1, text="📁 2. Cargar Excel Real", command=self.cargar_excel,
                                         fg_color="#00695C", hover_color="#004D40", height=45, font=ctk.CTkFont(size=14, weight="bold"))
        btn_cargar_excel.pack(side="left", padx=10, expand=True, fill="x")
        btn_estadisticas = ctk.CTkButton(frame_botones1, text="3. Dashboard Estadísticas", command=self.mostrar_estadisticas,
                                         fg_color="#1565C0", hover_color="#0D47A1", height=45, font=ctk.CTkFont(size=14, weight="bold"))
        ...
        btn_riesgo = ctk.CTkButton(frame_botones2, text="4. Lista completa en riesgo", command=self.lista_completa_riesgo,
                                   fg_color="#6A1B9A", hover_color="#4A148C", height=45, font=ctk.CTkFont(size=14, weight="bold"))
        btn_riesgo.pack(side="left", padx=10, expand=True, fill="x")
        btn_diagnostico = ctk.CTkButton(frame_botones2, text="5. Diagnóstico individual", command=self.diagnostico_individual,
                                        fg_color="#00838F", hover_color="#006064", height=45, font=ctk.CTkFont(size=14, weight="bold"))
        ...
        ctk.CTkLabel(frame_buscar, text="🔎 Buscar por Nombre o Cédula:", font=ctk.CTkFont(size=14)).pack(side="left", padx=10)
        self.entry_buscar = ctk.CTkEntry(frame_buscar, width=250, font=ctk.CTkFont(size=14))
        self.entry_buscar.pack(side="left", padx=5)
        btn_buscar = ctk.CTkButton(frame_buscar, text="Buscar y diagnosticar", command=self.buscar_y_diagnosticar,
                                   fg_color="#9C27B0", hover_color="#7B1FA2", height=35)
        btn_buscar.pack(side="left", padx=10)
        
        # Área de texto
        self.text_area = ctk.CTkTextbox(self.main_frame, height=350, font=ctk.CTkFont(family="Consolas", size=12), wrap="word")
    ...
    def mostrar_estadisticas(self):
        ...
        # Usar predicción del sistema en lugar de .deserto histórico
        ...
        info = f"👥 Total: {total}   |   ⚠️ En Riesgo (sistema): {desertores} ({tasa:.1f}%)   |   📝 Promedio Global: {promedio_gen:.1f}/20"
        ...
        # Gráfico de torta con la predicción
        ax1.pie([desertores, total - desertores], labels=['En Riesgo', 'Seguros'],
                colors=['#EF5350', '#66BB6A'], autopct='%1.1f%%', startangle=140)
        ax1.set_title('Distribución de Riesgo (Predicción)', color='white', pad=15, weight='bold')
        
        # Barras por rango de notas usando predicción
        ...
        for bajo, alto in rangos:
            ...
            valores.append(sum(1 for e in grupo if self.sistema.predecir(e)['deserta']))
        ...
        for bar in bars:
            y = bar.get_height()
            if y > 0:
                ax2.text(bar.get_x() + bar.get_width()/2, y + 0.5, int(y),
    ...
    def lista_completa_riesgo(self):
        ...
        # Calcular lista de estudiantes en riesgo usando predicción
        estudiantes_riesgo = []
        ...
        if not estudiantes_riesgo:
            self.escribir("✅ No hay estudiantes en riesgo según el sistema.")
            return
        
        estudiantes_riesgo.sort(key=lambda x: x[0].promedio_primer_semestre)
        
        self.escribir(f"{'='*60}")
        self.escribir(f"⚠️ LISTA COMPLETA DE ESTUDIANTES EN RIESGO ({len(estudiantes_riesgo)} total)")
        self.escribir("="*60)
        ...
        for i, (e, pred) in enumerate(estudiantes_riesgo, 1):
            ...
            self.escribir(f"   ID: {e.id} | Nota: {e.promedio_primer_semestre}/20 | Riesgo: {pred['riesgo']}")
            
            # Mostrar materias reprobadas (si las hay)
            if hasattr(e, 'notas') and e.notas:
                reprobadas = [m for m, n in e.notas.items() if n < 10]
                if reprobadas:
                    self.escribir(f"   📚 Materias reprobadas: {', '.join(reprobadas)}")
            
            # Mostrar factores de riesgo importantes
            factores = []
            ...
            if factores:
                self.escribir(f"   ⚠️ Factores: {', '.join(factores)}")
            
            # Mostrar primera recomendación si existe
            alerta = self.sistema.generar_alerta(e)
            if alerta and alerta.get('recomendaciones'):
                self.escribir(f"   💡 {alerta['recomendaciones'][0]}")
    ...
    def diagnostico_individual(self):
        ...
        try:
            ...
            materias_rep = [m for m, n in estudiante.notas.items() if n < 10] if hasattr(estudiante, 'notas') else []
            self.mostrar_perfil_mejorado(estudiante, diag, alerta, materias_rep)
        except ValueError:
    ...
    def buscar_y_diagnosticar(self):
        ...
        for idx, e in enumerate(resultados, 1):
            ...
            # Mostrar riesgo según predicción, no según histórico
            pred = self.sistema.predecir(e)
        ...
        if len(resultados) == 1:
            ...
            materias_rep = [m for m, n in est.notas.items() if n < 10] if hasattr(est, 'notas') else []
            self.mostrar_perfil_mejorado(est, diag, alerta, materias_rep)
        else:
            self.escribir("\n💡 Para diagnosticar a uno, anote su ID y use el botón 'Diagnóstico individual'")
    
    def mostrar_perfil_mejorado(self, estudiante, diag, alerta, materias_reprobadas=None):
        ...
        if hasattr(estudiante, 'cedula') and estudiante.cedula:
            texto = f"Cédula: {estudiante.cedula}"
            if hasattr(estudiante, 'carrera') and estudiante.carrera:
                texto += f"  |  Carrera: {estudiante.carrera}"
            ctk.CTkLabel(col_nom, text=texto, font=ctk.CTkFont(size=13)).pack(anchor="w")
        if hasattr(estudiante, 'turno') and estudiante.turno:
            texto = f"Turno: {estudiante.turno}"
            if hasattr(estudiante, 'seccion') and estudiante.seccion:
                texto += f"  |  Sección: {estudiante.seccion}"
            ctk.CTkLabel(col_nom, text=texto, font=ctk.CTkFont(size=13)).pack(anchor="w")
        
        # Etiqueta de riesgo
        if materias_reprobadas and len(materias_reprobadas) >= 2:
            riesgo_real = "ALTO"
            color_riesgo = "#EF5350"
            texto_riesgo = "⚠️ ALTO RIESGO (por materias reprobadas)"
        else:
            riesgo_real = diag.get('prediccion', 'BAJO')
            color_riesgo = "#EF5350" if riesgo_real == "ALTO" else "#66BB6A"
        ...
        if hasattr(estudiante, 'notas') and estudiante.notas:
            ...
            for i, (mat, nota) in enumerate(estudiante.notas.items()):
                ...
                frame_m = ctk.CTkFrame(parent, fg_color="#2b2b2b", corner_radius=10)
                frame_m.pack(fill="x", pady=4)
                ctk.CTkLabel(frame_m, text=f"{mat}:", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=10, pady=5)
                ctk.CTkLabel(frame_m, text=f"{nota} / 20", text_color=color_nota, font=ctk.CTkFont(weight="bold")).pack(side="right", padx=10, pady=5)
        else:
        ...
        fig, ax = plt.subplots(figsize=(5, 2.5), facecolor='#2B2B2B')
        if hasattr(estudiante, 'notas') and estudiante.notas:
        ...
        if materias_reprobadas and len(materias_reprobadas) >= 2:
            ctk.CTkLabel(frame_msg, text="⚠️ El estudiante tiene dos o más materias reprobadas. Se recomienda intervención urgente.", text_color="#FF9800", font=ctk.CTkFont(size=13, weight="bold")).pack(pady=5)
        elif riesgo_real == "ALTO":
